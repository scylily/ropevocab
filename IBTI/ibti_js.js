...
function initQuiz() {
  ...
  dimensions.forEach((dim) => {
    ...
    const selectedForDim = coreInDim.concat(
      shuffledNonCore.slice(0, Math.max(0, needed)),
    );
...
function showReview() {
  ...
  if (unanswered.length > 0) {
    ...
    const target =
      document.querySelector(`[data-qidx="${firstMiss}"]`) ||
      document.getElementById(`q-${firstMiss}`);
    ...
    .map((qData, i) => {
      ...
      if (userAnswers[i] && userAnswers[i].weight !== undefined) {
        ...
        if (w === 0) scoreLabel = `${sideA} +${(1.5 * multiplier).toFixed(1)}`;
          ...
          scoreLabel = `${sideA} +${(0.5 * multiplier).toFixed(1)}`;
        else if (w === 2)
          scoreLabel = `${sideA}/${sideB} 各+${(0.2 * multiplier).toFixed(1)}`;
        else if (w === 3)
          scoreLabel = `${sideB} +${(0.5 * multiplier).toFixed(1)}`;
        else if (w === 4)
          scoreLabel = `${sideB} +${(1.5 * multiplier).toFixed(1)}`;
      }
...
function generateReport() {
  ...
  userAnswers.forEach((ans, i) => {
    ...
    const isCore =
      q.isCore === true ||
      (typeof coreIndices !== "undefined" &&
        coreIndices.includes(q.originalId));
...
function downloadImage(id, name) {
  ...
  setTimeout(() => {
    html2canvas(element, {
      ...
      ignoreElements: (el) => {
        return (
          el.id === "image-processing-toast" ||
          el.id === "image-download-overlay"
        );
      },
    })
      .then((canvas) => {
